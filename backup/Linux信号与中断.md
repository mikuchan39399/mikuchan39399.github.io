...
我们平时用的系统调用，实际上并不是真正的系统调用接口，而是C语言封装好的系统调用库
内核中存在一个系统调用函数指针数组，它们指向真正的系统调用
在我们调用系统库函数时，C库会帮我们找到系统调用名对应的系统调用号，也就是那个指针数组下标，并把这个系统调用号写入**通用寄存器**中，`mov n eax`，然后调用 `syscall` 或 `int 0x80` 陷入内核态，对应的中断执行代码则会拿到寄存器中的系统调用号，去查系统调用表并执行

...
6. CPU 陷入内核态，根据中断号，去中断向量表中查表，执行对应的操作系统指令集，以回应不同的中断需求
...
> [!Important]CPU 主频决定了指令执行有多快，而时钟中断频率由操作系统规定

有了周期性的时钟中断，CPU 就能每隔一段时间自动陷入内核态，完成基本的信号检测
而时钟中断对应的中断向量，也就是对应时钟中断的指令集，就是完成操作系统基本生命活动的指令集
[!Important]
...
随后，CPU 去中断向量表中寻找时钟中断对应的中断处理程序（ISR）并执行。这段内核代码承载了操作系统最基础的生命活动：它不仅会顺手检查并处理当前进程的未决信号，还会更新系统时间、扣减当前进程的时间片，并在时间片耗尽时触发进程调度。没有此时钟节拍，整个操作系统的运转就会陷入停滞。

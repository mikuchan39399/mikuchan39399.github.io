...
> [!Note]
> **信号处理过程中第四步实际上有些复杂**
> 为了让用户态的普通`handler`函数可以正常执行，并且结束后把控制权交还给被中断的主程序，操作系统会在切回用户态之> > 前，对当前进程的用户栈做点小手脚：
> 1. 保存现场，程序最初被打断陷入内核态时，CPU 硬件和操作系统的中断处理程序，已经将主程序的全部硬件上下文（原程序> 计数器 EIP、ESP、EFLAGS、各种通用寄存器）安全地保存在了内核栈中
> 2. 构建信号栈帧：为了在 `handler` 执行完后能恢复主程序，内核提取**内核栈**中保存的上下文快照`ucontext` ，将其压入 **》> 用户栈**。同时，内核会在用户栈上布置好 handler 需要的参数（如信号编号 `signo`）
> 3. 压入用户栈跳转代码：内核在**用户栈**的栈顶继续压入一个伪造的返回地址。这个地址指向一段用于触发 `sigreturn` 系统 > 调用的跳转代码
> 4. 篡改返回路线：内核直接去修改此刻仍保存在**内核栈**上的寄存器快照，把即将被恢复的 `EIP` 强行覆写为 `handler` 函数 > 的入口地址，把 `ESP` 覆写为刚刚布置好的用户栈新栈顶。
> 5. 降级跃迁：内核执行 `iret` 指令，CPU 从**内核栈**中弹出被篡改过的状态，特权级降回 `CPL=3`。此时程序完美降落回**用> 户态**，毫不违和地开始执行 `handler`，而它操作的正是刚刚精心伪造过的**用户栈**

...
>   通用寄存器的保存不归 CPU 硬件管，中断处理程序的开头会调用 `pusha` 或 `pushad` 等指令，把这些寄存器全压入内核栈兜底。
...
> [!Note]
> **信号处理的“蹦床魔法”：从内核态执行用户态 Handler 的复杂内幕**
> 
> 为了让用户态的普通 `handler` 函数可以正常执行，并在结束后把控制权平滑交还给被中断的主程序，操作系统会在切回用户态之前，对当前进程的**用户栈**做一套极其精妙的“小手脚”：
> 
> 1. **保存现场 (内核栈兜底)**：程序最初被打断并陷入内核态时，CPU 硬件和操作系统的中断处理程序，已经将主程序的全部硬件上下文（原程序计数器 `EIP`、`ESP`、`EFLAGS`、各种通用寄存器等）安全地保存在了**内核栈**中。
> 2. **构建信号栈帧 (备份至用户栈)**：为了在 `handler` 执行完后能找回回家的路，内核会提取**内核栈**中保存的上下文快照 `ucontext`，并将其压入**用户栈**。同时，内核会在用户栈上布置好 `handler` 需要的参数（如信号编号 `signo`）。
> 3. **布置“蹦床” (压入跳转代码)**：内核继续在**用户栈**的栈顶压入一个伪造的返回地址。这个地址并不指向主程序的原位置，而是指向一段用于触发 `sigreturn` 系统调用的特殊跳转代码（也就是咱们常说的“蹦床”）。
...
> 6. **蹦床跃迁与满血复活 (sigreturn)**：当 `handler` 遇到 `return` 时，它会从用户栈弹出那个伪造的蹦床地址并跳过去。蹦床代码顺势执行 `syscall` 或 `int 0x80` 再次陷入内核。此时内核执行 `sys_sigreturn`，去用户栈里把步骤 `2` 中备份的 `ucontext` 挖出来，直接覆盖到硬件寄存器上。最后再执行一次 `iret`，主程序就在原地址满血复活啦！
...
```
现在，操作系统开始从根目录进行路径解析的时候，优先使用本目录的`dentry`，**内存读写速度可比磁盘IO快多了**
路径尽头的文件就是路径缓存树的叶子节点，也存在对应的`dentry`
...
若挂载成功，这时缓存目录树中一定已经有了那个被挂载目录的 `dentry`，我们叫它`old_dentry`
内核会在 `old_dentry` 中的 `d_flags` 标志位设置已挂载，这个标志位一旦被设置成这样就会告知操作系统接下来的路径解析不要走原先的 `dentry` 树路径了，我们来走跨纬度传送门吧，`lookup_mnt()` 这个函数就是传送门，他会让操作系统去查全局挂载表
```cpp
struct vfsmount {
...
```
全局挂载表是个哈希，它的 key 是一个 <父分区的 `vfsmount`, 挂载点的 `old_dentry`>，value 是新分区的 `vfsmount`
这样，操作系统就不会管 `old_dentry`了，直接用上新分区里的 `mnt_root` 就行，`mnt_root` 是被挂载的分区的根目录 `dentry`
...
- 不可跨分区，inode 不可跨分区
- 严禁用户自己硬链接目录，**那样会形成目录环，导致路径解析陷入死循环**

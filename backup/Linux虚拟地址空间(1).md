...
char *str = "Hello"; //实际上，字符串字面量"Hello"存在于.rodata中，指针变量 ptr 作为普通变量存在于其他区域中
```
...
#### 栈区与函数栈帧
当一个函数被调用时，操作系统会在栈区为这个函数分配一块连续的内存空间，这块空间就叫函数栈帧
...
- 特点：
   - 操作系统全权交由用户管理，不使用 free 等接口释放内存的话会导致内存泄露
   - 空间相较与栈区会大很多
 > 堆区和栈区两面包夹芝士中间还有一块空白区，被称作共享区，用于加载动态库(.so / .dll) 或进行文件映射
### 引入虚拟地址空间

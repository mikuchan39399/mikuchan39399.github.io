...
在`Linux`内核中, PCB被落地为具体的 `task_struct` 结构体
`Linux`中的进程 相当于 被加载进内存的 `task_struct` 与具体的代码和数据

#### 组织进程
`Linux`中以双链表的形式组织管理着 `task_struct` 以管理进程, 而不用关心具体的代码数据
其中,`Linux`采用侵入式数据结构的设计来让 `task_struct` 灵活的被挂载进多个数据结构中
...
#### 创建进程-`fork`系统接口
`fork`是`Linux`的`unistd.h`所包含的系统接口, 调用后以运行程序的进程为父进程产生一个独立的子进程
若`fork`成功, 父进程的fork返回值为子进程的`pid`, 子进程的`fork`返回值为`0`
若`fork`失败, 不产生子进程, fork返回值为`-1`
所以可以用分支语句根据返回值来区分父子进程

#### 进程工作目录
Linux 中每个进程都有一个当前工作目录`（cwd）`，被作为进程信息的一部分存储在`PCB`里，具体是`fs_struct`指针
子进程的`cwd`默认继承父进程，而由命令行工具bash所创建的进程的父进程全都是`bash`自己。
所以， 进程的`cwd`默认是使用`bash`的目录
在程序中使用`chdir(const char *path)`系统接口可以改变当前进程的工作目录
> [!NOTE]
...
| **S** | 睡眠状态 | Sleeping | 意味着进程在等待事件完成。这里的睡眠有时候也叫做**可中断睡眠** 。 | 
| **D** | 磁盘休眠状态 | Disk Sleep | 有时候也叫**不可中断睡眠状态** ，在这个状态的进程通常会**等待 I/O 的结束**。 |
...
> [!NOTE]
> 当父进程早于子进程退出时, 子进程会被 `init `进程`(pid=1)` 领养, 成为孤儿进程, 孤儿进程可以是任何状态, 属于正常现象
> 僵死进程会以终⽌状态保持在进程表中，并且会⼀直在等待⽗进程读取退出状态代码。
> 直到父进程退出，僵尸进程会被 `init` 进程领养。`init` 进程会立即执行回收操作，将该僵尸进程彻底释放。

> [!WARNING]
> 僵死进程会挤占内存的PCB资源, 包括`task_struct`和进程号,会导致内存的浪费和进程号的挤兑，导致创建不出新的进程 注意父> 进程一定要使用wait相关系统接口
